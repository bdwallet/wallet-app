<template>
	<div class="paper-shareout">
		<div class="gosupects">
			<div class="sprites-share-bg1"></div>
			<div class="sprites-share-supects" @click="$router.push({path:'/archives'})"></div>
		</div>
    <div class="content">
    	...
    	<div class="marking">
			...
			<div class="btn-group">
				<div class="sprites-orange-btn2" @click="gohome(figureData)">查看更多情报</div>
				<div class="sprites-blue-btn2" @click="goWrite()">提供新情报</div>
			</div>
...
<script>
	...
	export default {
	    ...
	    doLike(item){
			...
			goWrite(){
				if(this.$route.params.userId==this.$store.getters.userInfo.user_id){
						this.$dialog.tip({
							type: 'other',
							content: '您提交过该大佬得评价，请勿重复提交',
						});
					 return;
				}else{
					this.$router.push(`/write/${this.figureData.figure_id}`);
				}
			}
		},
		beforeMount() {},
