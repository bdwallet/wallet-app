    ...
    <div class="container">
      <div class="search-arer">
        ...
        <div class="search-box">
          <h1 class="total">
            {{ $lang("共{total}项目", { total: itemTotal }) }}
          </h1>
      ...
      <scroll v-if="itemData && itemData.length > 0" :on-infinite="onInfinite">
        <transition-group tag="ul" class="ratings-list-box">
          <li
            ...
            :key="item"
            @click="$router.push(`/detail/${item.item_id}`)"
          >
            <circle-progress
              class="score"
            ...
            <div class="info">
              ...
              <div class="name">
                <h1>
                  {{ item.item_urrency_abbreviation
                  }}{{
                    item.item_urrency_abbreviation && item.item_cn_name
                      ? "/"
                      : ""
                  }}{{ item.item_cn_name }}
                </h1>
      ...
      <empty
        v-if="!loading && (!itemData || (itemData && itemData.length == 0))"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //搜索
    searchItem() {
      if (this.keyword && this.keyword != this.item) {
        //有关键词，并且与上一次搜索内容不一样
        this.current_page = 1;
        this.item = this.keyword;
        this.getItemList();
      } else if (!this.keyword && this.item) {
        //没有关键词，并且存在上一次搜索 =》 清空搜索并重置列表
        this.current_page = 1;
        this.item = "";
        this.getItemList();
      }
